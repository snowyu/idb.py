...
echo benchmark the  get key/value, run $TESTPROG loop $LOOP, repeat $REPEAT, total $COUNT
#echo clear DB first
#redis/redis-cli -p 3333 flushdb
#python -m timeit -s 'from '$TESTPROG' import a' -n 1 -r 1 'a.test_get()'
...
do
  I=$(( $I + 1 ))
  ST=`s_time`
  RESULT=$(python -m timeit -v -s 'from '$TESTPROG' import a' -n $LOOP -r $REPEAT 'a.test_get()')
  ET=`s_time`
  echo $I. $RESULT
